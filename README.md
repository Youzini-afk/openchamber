...
**Safe Dockerfile builds for agents:** The Docker image includes a `docker` compatibility wrapper for shared-host deployments. It supports `docker build ...` and `docker buildx build ...` through daemonless BuildKit without mounting `/var/run/docker.sock` or exposing the host Docker daemon. By default, builds run as the non-root `openchamber` user and use cache-only output to validate Dockerfiles; use local artifact outputs such as `--output type=oci,dest=/tmp/image.tar` if you need an artifact. Registry push outputs are blocked. Runtime commands such as `docker run`, `docker compose`, `docker ps`, `docker exec`, `docker pull`, and `docker push` are intentionally rejected. This keeps agent Dockerfile validation available while avoiding host-level Docker control on shared servers.

If your runtime supports rootless user namespaces, you can opt into RootlessKit networking by setting `ROOTLESSKIT='rootlesskit --net=slirp4netns --copy-up=/etc --disable-host-loopback'`. The default remains daemonless/non-root without RootlessKit so it works on restricted cloud builders and shared-host containers.
