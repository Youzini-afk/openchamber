...
type ProxyRuntimeDeps = {
  tryHandleLocalFsProxy: (method: string, requestPath: string, headers?: Record<string, string>) => Promise<ApiProxyResponsePayload | null>;
...
export async function handleProxyBridgeMessage(
  ...
  switch (type) {
    ...
    case 'api:proxy': {
      ...
      const localFsResponse = await deps.tryHandleLocalFsProxy(normalizedMethod, normalizedPath, headers);
      if (localFsResponse) {
