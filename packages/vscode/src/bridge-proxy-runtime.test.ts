...
describe('VS Code API proxy local fs header pass-through', () => {
  test('forwards request headers to the local fs proxy', async () => {
    const calls: { method: string; requestPath: string; headers?: Record<string, string> }[] = [];
    const localFsDeps = {
      ...deps,
      tryHandleLocalFsProxy: async (method: string, requestPath: string, headers?: Record<string, string>) => {
        calls.push({ method, requestPath, headers });
        return null;
      },
    };
    ...
    await handleProxyBridgeMessage(
      {
        ...
        payload: {
          method: 'GET',
          path: '/api/fs/stat?path=foo.txt',
          headers: { 'x-opencode-directory': '/worktree-a', accept: 'application/json' },
        },
      },
      ctx,
      localFsDeps,
    ...
    assert.equal(calls[0]?.requestPath, '/api/fs/stat?path=foo.txt');
    assert.equal(calls[0]?.headers?.['x-opencode-directory'], '/worktree-a');
