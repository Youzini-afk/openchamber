...
export const CommandPalette: React.FC = () => {
  ...
  const commands = React.useMemo<CommandEntry[]>(() => {
    const list: CommandEntry[] = [
      ...
      },
      {
        ...
        onSelect: run(() => {
          if (effectiveDirectory) openContextOverview(effectiveDirectory);
        }),
      },
      {
        id: 'open-settings',
  ...
  const settingsEntries = React.useMemo<CommandEntry[]>(() => {
    return SETTINGS_PAGE_METADATA
      ...
      .map((page) => {
        const SettingsIcon = getSettingsNavIcon(page.slug);
        const keywords = (page.keywords ?? []).join(' ');
        return {
          id: `settings:${page.slug}`,
          title: page.title,
          icon: SettingsIcon ? <SettingsIcon className="mr-2 h-4 w-4" /> : <Icon name="settings-3" className="mr-2 h-4 w-4" />,
