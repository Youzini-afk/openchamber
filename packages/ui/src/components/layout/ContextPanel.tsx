...
const getModeLabel = (
  ...
  if (mode === 'browser') return t('contextPanel.browser.open');
...
const getTabIcon = (tab: { mode: 'diff' | 'file' | 'context' | 'plan' | 'chat' | 'preview' | 'browser'; targetPath: string | null }): React.ReactNode | undefined => {
  if (tab.mode === 'file') {
...
const isLoopbackBrowserUrl = (value: string): boolean => {
  try {
...
const IframeBrowserPane: React.FC<DesktopBrowserPaneProps> = ({ initialUrl, directory, tabID }) => {
  ...
  React.useEffect(() => {
    ...
    if (!isLoopbackBrowserUrl(currentUrl)) {
      setProxyState({ status: 'error', message: '' });
      setIsLoading(true);
      return;
    }
    ...
    void (async () => {
      try {
        const response = await fetch('/api/preview/targets', {
          ...
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({ url: currentUrl, allowExternal: false }),
        });
