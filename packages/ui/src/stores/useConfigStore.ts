            ...
            try {
                ...
                if (data) {
                    ...
                    const autoUpdateChecksEnabled = typeof data?.autoUpdateChecksEnabled === 'boolean'
                        ...
                        : undefined;
                    const sttProvider = data?.sttProvider === 'browser' || data?.sttProvider === 'server' || data?.sttProvider === 'wasm'
                        ? data.sttProvider
                        : undefined;
                    const sttServerUrl = typeof data?.sttServerUrl === 'string' ? data.sttServerUrl : undefined;
                    const sttModel = typeof data?.sttModel === 'string' ? data.sttModel : undefined;
                    const sttLanguage = typeof data?.sttLanguage === 'string' ? data.sttLanguage : undefined;
...
if (!unsubscribeConfigStoreChanges) {
    unsubscribeConfigStoreChanges = subscribeToConfigChanges(async (event) => {
        ...
        if (scopeMatches(event, "providers")) {
            ...
            tasks.push(loadProviders({ force: true, source: 'configChange:providers' }));
        }
