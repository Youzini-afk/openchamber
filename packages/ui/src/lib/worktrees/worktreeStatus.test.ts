...
// Import via a cache-busting query so a previous test file that already
// loaded worktreeStatus (with the real gitApi) cannot pollute this file.
const worktreeStatusPath = './worktreeStatus?test-isolate';
const worktreeStatusMod = await import(worktreeStatusPath) as typeof import('./worktreeStatus');
const { getRootBranch, invalidateResolvedProjectRootCache } = worktreeStatusMod;
