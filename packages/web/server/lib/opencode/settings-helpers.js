export const createSettingsHelpers = (dependencies) => {
  ...
  const sanitizeSettingsUpdate = (payload) => {
    ...
    if (typeof candidate.sttServerUrl === 'string') {
      result.sttServerUrl = candidate.sttServerUrl.trim().slice(0, STT_SERVER_URL_MAX_LENGTH);
    }
    if (typeof candidate.sttModel === 'string') {
      result.sttModel = candidate.sttModel.trim().slice(0, STT_MODEL_MAX_LENGTH);
    }
    if (typeof candidate.wasmSttModel === 'string') {
      result.wasmSttModel = candidate.wasmSttModel.trim().slice(0, STT_MODEL_MAX_LENGTH);
    }
    if (typeof candidate.sttLanguage === 'string') {
      result.sttLanguage = candidate.sttLanguage.trim().slice(0, STT_LANGUAGE_MAX_LENGTH);
    }
