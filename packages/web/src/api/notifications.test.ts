...
describe('web notifications API', () => {
  ...
  it('defers hidden-page notification delivery to active push subscription without claiming foreground delivery', async () => {
    ...
    await expect(api.notifyAgentCompletion({ title: 'Ready', body: 'Done', tag: 'ready-session-visible' })).resolves.toBe(true);

    expect(showNotification).toHaveBeenCalledTimes(1);
    expect(showNotification).toHaveBeenCalledWith('Ready', expect.objectContaining({ body: 'Done', tag: 'ready-session-visible' }));
    expect(created).toHaveLength(0);
  });
});
